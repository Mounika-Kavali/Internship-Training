...
export class BarCodeReader extends Component {
    ...
    composeLotNumberConfirmationMessage() {
      ...
      return <div>
          <h5>This is not the recommended Lot Number for Part Number&nbsp;{partNumber}&nbsp;but you may still proceed.</h5>
    ...
    confirmNonRecommendedBarcodeHandler() {
      ...
      if (lotList && lotList.includes(lot)) {
        ...
        this.setState({showLotNumberConfirmation_Popup:false,inValidLot_Popup:true,notAvailableLot:lot,});
      }
      this.setState({ inputValue: '',});
    }
    ...
    dispatchManualBarcode() {
      let [part, lot] = this.state.inputValue.split('_');
      let containerIndex= jsonData.catalog.containers.findIndex((x)=>x.statistics.part_number === part);
      ...
      else if(jsonData.catalog.containers[containerIndex].lot_number===null){
        let matchingIndex = jsonData.catalog.total_lots.recommended_lot_list.findIndex(
          (recommendedLot) =>
      ...
      else if(jsonData.catalog.containers[containerIndex].lot_number==="OI"){
        if(jsonData.catalog.total_lots.recommended_lot_list.findIndex(
          (recommendedLot) =>
            recommendedLot.partnumber === part && recommendedLot.lot === "custom"
        )>-1){
        this.props.onScan(this.state.inputValue);
        this.setState({ inputValue: '',});
        }
  ...
  render() {
    const {inValidLot_Popup,notAvailableLot,inValidBarcode_Popup,inputValue,showLotNumberConfirmation_Popup}=this.state;
    return (
      <>
        ...
        <LotConfirmModal
          ...
          okHandler={() => { this.confirmNonRecommendedBarcodeHandler() }}
          cancelHandler={() => { this.cancelNonRecommendedBarcodeHandler() }}
